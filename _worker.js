...
async function fetchSheetsData(env) {
  ...
  const [sellCasesId, buyCasesId, sellInqId, buyInqId, paymentRange] = await Promise.all([
    ...
    getRawValue(env, 'GOOGLE_SHEETS_PAYMENT_RANGE'),
  ]);

  const effectivePaymentRange = paymentRange || '入金確認!A1:Z200';

  const [sellCases, buyCases, sellInquiries, buyInquiries, salesSummary, rawPayment] = await Promise.all([
  ...
  return { sellCases, buyCases, sellInquiries, buyInquiries, salesSummary, paymentRecords };
}
